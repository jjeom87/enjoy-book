...
![웹 클라이언트와 웹 서버](/images/네트워크_해킹_보안/네트워크일반/HTTP완벽가이드/figure_1-1.png)
...
![웹 리소스란 웹에 콘텐츠를 제 공하는 모든 것을 말한다](/images/네트워크_해킹_보안/네트워크일반/HTTP완벽가이드/figure_1-2.png)
...
![HTTP 메시지는 단순한 줄 단위 텍스트로 구조를 갖고 있다](/images/네트워크_해킹_보안/네트워크일반/HTTP완벽가이드/figure_1-7.png)
...
##### 1.5.1 간단한 메세지의 예
![http;//www.joes-hardware.com/tools.html에 대한 GET 트랜잭션 의 예](/images/네트워크_해킹_보안/네트워크일반/HTTP완벽가이드/figure_1-8.png)
...
![HTTP 네트워크 프로토콜 스택](/images/네트워크_해킹_보안/네트워크일반/HTTP완벽가이드/figure_1-9.png)
...
![웹브라우저 연결의 기본적인 절차](/images/네트워크_해킹_보안/네트워크일반/HTTP완벽가이드/figure_1-10.png)
...
* HTTP/0.9
  ...
  * HTTP/0.9는 오직 GET 메서드만 지원하고, 멀티미디어 콘텐츠에 대한 M]ME 타입이나, HTTP 헤더. 버전 번호는 지원하지 않음
  * HTTP/0.9는 원래 간단한 HTML 객체를 받아 오기 위해 만들어진 것
  * HTTP/0.9는 금방 HTTP/1.0으로 대체  
* HTTP/1.0 
  * HTTP/1.0은 버 전 번호, HTTP 헤더, 추가 메서드, 멀티미디어 객체 처리를 추가
  * HTTP가 상업적, 학술적으로 급성장하던 시 기 에 만들어 진, 잘 동작하는 용례들의 모음에 가깝움
* HTTP/1.0+
